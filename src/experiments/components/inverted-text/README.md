...
is filled white and subtracts whatever color sits behind it, so it reads white on dark, black
on light, and shifts hue over saturated colors.

## Layout

The promotable piece lives at the top level: `InvertedStage.tsx` + `StageBackground.tsx` +
`modes.ts` + `invert.css` (the stage renders the word over a mode-driven backdrop; `InvertedStage`
imports its own CSS). Everything that only exists to *drive* the demo — the mode/speed/lightness
controls and the "essentials" code viewer — lives in `controls/` and is not part of a promotion.

## Controls

Four mode buttons plus one contextual slider that swaps meaning with the mode:
...
- **Dark → Light** — the slider becomes a **lightness** control driving a neutral grey backdrop
  from black to white.
- **Blue** / **Magenta** — snap the backdrop to a saturated flat color (~200ms transition) to
  show the hue subtraction (white − blue ≈ yellow, white − magenta ≈ green). No slider.
